...
CANONICAL_LOGO = ROOT / "app" / "static" / "img" / "brand-primary.svg"
CANONICAL_SYMBOL = ROOT / "app" / "static" / "img" / "brand-symbol.svg"
SITE_INDEX = ROOT / "site" / "index.html"
SITE_STYLES = ROOT / "site" / "styles.css"
SITE_OVERLAY = ROOT / "site" / "styles" / "v2.1-public.css"
SITE_HERO = ROOT / "site" / "sections" / "hero-trust.html"
SITE_LOGO = ROOT / "site" / "assets" / "logo.svg"
SITE_SYMBOL = ROOT / "site" / "assets" / "symbol.svg"
...
def test_github_pages_preview_tracks_v212_navigation_brand_and_mobile_overlay() -> None:
    ...
    hero = SITE_HERO.read_text(encoding="utf-8")
    ...
    assert "@media(max-width:620px)" in overlay
    assert "assets/symbol.svg" in hero
    assert ">CTH<" not in hero
    assert SITE_LOGO.read_bytes() == CANONICAL_LOGO.read_bytes()
    assert SITE_SYMBOL.read_bytes() == CANONICAL_SYMBOL.read_bytes()
