...
def test_v044_seeds_five_sector_cases_with_complete_demo_workflows():
    with SessionLocal() as session:
        ...
        for company in by_name.values():
            assert company["activity_records"] > 0
            assert company["calculations"] > 0
            assert company["evidence"] > 0
            assert company["implementation_plans"] > 0
            assert company["demo_story"]["headline"]
        assert len({company["current_stage"] for company in by_name.values()}) >= 3
        assert by_name["Hotel Bosque Azul"]["latest_inventory_status"] == "Aprobado"
...
def test_v044_demo_center_can_switch_between_companies():
    with TestClient(app) as client:
        ...
        assert "Cinco empresas, cinco recorridos distintos" in page.text
        assert "Café Sierra Verde" in page.text
        assert "Ruta Norte Logística" in page.text
        assert "Hotel Bosque Azul" in page.text
        with SessionLocal() as session:
...
def test_v044_demo_preparation_is_idempotent():
    ...
    with SessionLocal() as session:
        summary = demo_environment_summary(session)
        assert summary["organization_count"] == 5
        assert summary["totals"]["activity_records"] == 290
        assert summary["totals"]["calculations"] == 338


def test_v044_mac_demo_command_is_self_contained():
    root = Path(__file__).resolve().parents[1]
    if not (root / "15_PREPARAR_Y_CERTIFICAR_DEMO.command").is_file():
        pytest.skip("Prueba exclusiva del paquete macOS")
