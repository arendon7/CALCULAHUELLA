...
def test_product_readiness_authority_covers_current_repository() -> None:
    ...
    assert discovered
    assert all(
        Path(path).name.startswith(("test_v260", "test_v261", "test_v262"))
...
def test_product_readiness_authority_rejects_stale_or_missing_classification(
    ...
    _write(tmp_path, "tests/test_v262_known.py")
    ...
    assert "Contrato clasificado inexistente: tests/test_v262_missing.py" in message
...
def test_product_readiness_authority_rejects_duplicates_and_overlap(tmp_path: Path) -> None:
    ...
    with pytest.raises(ContractAuthorityError) as exc_info:
        validate_authority(
            tmp_path,
            targeted=("tests/test_v262_known.py", "tests/test_v262_known.py"),
            transversal=("tests/test_v262_known.py",),
            excluded={},
        )
...
def test_product_readiness_authority_requires_reason_for_exclusion(tmp_path: Path) -> None:
    _write(tmp_path, "tests/test_v262_known.py")
    _write(tmp_path, "tests/test_v262_auxiliary.py")

    with pytest.raises(ContractAuthorityError, match="Exclusión sin justificación"):
        validate_authority(
            tmp_path,
            targeted=("tests/test_v262_known.py",),
            transversal=(),
            excluded={"tests/test_v262_auxiliary.py": "   "},
        )
...
def test_product_readiness_authority_accepts_explicit_reasoned_exclusion(
    ...
    discovered = validate_authority(
        ...
        transversal=(),
        excluded={
            "tests/test_v262_auxiliary.py": "Auxiliary harness contract covered by its canonical replacement."
        },
    )

    assert discovered == (
        "tests/test_v262_auxiliary.py",
        "tests/test_v262_known.py",
    )
...
def test_product_readiness_execution_plan_keeps_transversal_contracts_isolated() -> None:
    plan = execution_plan(
        targeted=("tests/test_v2606_known.py", "tests/test_v261_known.py", "tests/test_v262_known.py"),
