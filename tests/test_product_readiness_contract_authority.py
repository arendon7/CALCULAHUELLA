...
def test_product_readiness_authority_covers_current_repository() -> None:
    ...
    assert discovered
    assert all(
        Path(path).name.startswith(("test_v260", "test_v261", "test_v262", "test_v263"))
    ...
    assert "tests/test_v262_mobile_operational_priority.py" in discovered
    assert "tests/test_v262_mobile_operational_priority.py" in TARGETED_CONTRACTS
    assert "tests/test_v263_mac_rc_provenance_continuity.py" in discovered
    assert "tests/test_v263_mac_rc_provenance_continuity.py" in TARGETED_CONTRACTS
    assert "test_v260*.py" in VERSIONED_GLOBS
    assert "test_v261*.py" in VERSIONED_GLOBS
    assert "test_v262*.py" in VERSIONED_GLOBS
    assert "test_v263*.py" in VERSIONED_GLOBS
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("tests/test_v262_known.py", "tests/test_v26299_future_contract.py"),
        ("tests/test_v263_known.py", "tests/test_v26399_future_contract.py"),
    ),
)
...
def test_product_readiness_authority_rejects_stale_or_missing_classification(
    ...
    _write(tmp_path, "tests/test_v263_known.py")

    with pytest.raises(ContractAuthorityError) as exc_info:
        validate_authority(
            tmp_path,
            targeted=(
                "tests/test_v263_known.py",
                "tests/test_v263_missing.py",
            ),
            transversal=(),
            excluded={},
        )
    ...
    assert "Contrato clasificado inexistente: tests/test_v263_missing.py" in message
...
def test_product_readiness_authority_rejects_duplicates_and_overlap(tmp_path: Path) -> None:
    ...
    with pytest.raises(ContractAuthorityError) as exc_info:
        validate_authority(
            tmp_path,
            targeted=("tests/test_v263_known.py", "tests/test_v263_known.py"),
            transversal=("tests/test_v263_known.py",),
            excluded={},
        )
...
def test_product_readiness_authority_requires_reason_for_exclusion(tmp_path: Path) -> None:
    _write(tmp_path, "tests/test_v263_known.py")
    _write(tmp_path, "tests/test_v263_auxiliary.py")

    with pytest.raises(ContractAuthorityError, match="Exclusión sin justificación"):
        validate_authority(
            tmp_path,
            targeted=("tests/test_v263_known.py",),
            transversal=(),
            excluded={"tests/test_v263_auxiliary.py": "   "},
        )
...
def test_product_readiness_authority_accepts_explicit_reasoned_exclusion(
    ...
    discovered = validate_authority(
        ...
        transversal=(),
        excluded={
            "tests/test_v263_auxiliary.py": "Auxiliary harness contract covered by its canonical replacement."
        },
    )

    assert discovered == (
        "tests/test_v263_auxiliary.py",
        "tests/test_v263_known.py",
    )
...
def test_v26013_gate_triggers_for_current_versioned_contract_families() -> None:
    source = V26013_WORKFLOW.read_text(encoding="utf-8")

    assert '- "tests/test_v260*.py"' in source
    assert '- "tests/test_v261*.py"' in source
    assert '- "tests/test_v262*.py"' in source
    assert '- "tests/test_v263*.py"' in source
